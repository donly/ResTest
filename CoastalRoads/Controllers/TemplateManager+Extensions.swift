...
extension TemplateManager {
    ...
    final func installBarButtons() {
        ...
        panButton.image = UIImage(named: "Pan", in: .main, compatibleWith: mapViewController.traitCollection)
        let destsButton = CPBarButton(type: .image) { (_) in
            self.showGridTemplate()
        }
        destsButton.image = UIImage(named: "Favorites", in: Bundle.main, compatibleWith: mapViewController.traitCollection)
